...
const BarChart = (0, import_index_12fa369c.c)(($$result, $$props, $$bindings, slots) => {
  let zDomain;
  let xScale;
  let data_grouped;
  let { data } = $$props;
  ...
  return `${zDomain[1] ? `<ul class="${"legend-block svelte-1ri9pb8"}">${(0, import_index_12fa369c.r)(zDomain, (group, i) => {
    return `<li class="${"svelte-1ri9pb8"}"><div class="${"legend-vis " + (0, import_index_12fa369c.h)(i == 0 ? "bar" : "marker") + " svelte-1ri9pb8"}"${(0, import_index_12fa369c.u)({
      "height": `1rem`,
      "width": i == 0 ? "1rem" : markerWidth + "px"
    })}></div>
		<span class="${(0, import_index_12fa369c.h)((0, import_index_12fa369c.w)(i == 0 ? "bold" : "brackets")) + " svelte-1ri9pb8"}">${(0, import_index_12fa369c.h)(group)}</span>
	</li>`;
  })}</ul>` : ``}

${(0, import_index_12fa369c.r)(data_grouped, (group) => {
    return `<div class="${"label-group svelte-1ri9pb8"}">${(0, import_index_12fa369c.h)(group.label)}
		${(0, import_index_12fa369c.r)(group.values, (d, i) => {
      return `<span class="${"label " + (0, import_index_12fa369c.h)(i == 0 ? "bold" : "sml brackets") + " svelte-1ri9pb8"}">${(0, import_index_12fa369c.h)(formatTick(xScale(d[xKey])))}${(0, import_index_12fa369c.h)(suffix)}</span>`;
    })}</div>
	<div class="${"bar-group svelte-1ri9pb8"}"${(0, import_index_12fa369c.u)({ "height": `${barHeight}px` })}>${(0, import_index_12fa369c.r)(group.values, (d, i) => {
      return `${i == 0 ? `<div class="${"bar svelte-1ri9pb8"}"${(0, import_index_12fa369c.u)({
        "left": `0`,
        "width": `${xScale(d[xKey])}%`
      })}></div>` : `<div class="${"marker svelte-1ri9pb8"}"${(0, import_index_12fa369c.u)({
