...
async function init_draw() {
  (0, import_index_e390a0ae.g)(import_mapstore.mapobject).addSource("drawsrc", {
    type: "geojson",
    data: {
      type: "Feature",
      geometry: {
  ...
  (0, import_index_e390a0ae.g)(import_mapstore.mapobject).addLayer({
    id: "draw_layer",
    type: "line",
    source: "drawsrc",
    paint: {
  ...
  import_mapstore.draw_type.subscribe(() => {
    coordinates = [];
    circle_paint((0, import_index_e390a0ae.g)(import_mapstore.draw_type) != "radius");
  });
  import_mapstore.draw_type.set("radius");
  import_mapstore.radiusInKm.subscribe(circle_paint);
  (0, import_index_e390a0ae.g)(import_mapstore.mapobject).on("click", "bounds", function boundclick(e) {
    console.log(e.lngLat, (0, import_index_e390a0ae.g)(import_mapstore.draw_type));
    switch ((0, import_index_e390a0ae.g)(import_mapstore.draw_type)) {
      case "radius":
  ...
  (0, import_index_e390a0ae.g)(import_mapstore.mapobject).on("zoomend", function() {
    import_mapstore.draw_enabled.set((0, import_index_e390a0ae.g)(import_mapstore.mapobject).getZoom() < 10);
  });
  (0, import_index_e390a0ae.g)(import_mapstore.mapobject).on("mousemove", "bounds", function move(e) {
    switch ((0, import_index_e390a0ae.g)(import_mapstore.draw_type)) {
      case "radius":
...
function change_data(layer, data) {
  (0, import_index_e390a0ae.g)(import_mapstore.mapobject).getSource(layer).setData(data);
}
function update(coordinates2) {
  const bbox = getbbox(coordinates2);
  const features = (0, import_index_e390a0ae.g)(import_mapstore.mapobject).queryRenderedFeatures(bbox.map((d) => (0, import_index_e390a0ae.g)(import_mapstore.mapobject).project(d)), { layers: ["centroids"] });
  const oa = features.filter((i) => inPolygon(coordinates2, i.geometry.coordinates)).map((d) => d.properties.id);
  var current = (0, import_index_e390a0ae.g)(import_mapstore.selected);
  var last = current[current.length - 1];
  bbox.map((d) => {
    last.lat.push(d[1]);
    last.lng.push(d[0]);
  });
  if ((0, import_index_e390a0ae.g)(import_mapstore.add_mode)) {
    current.push({
  ...
  console.warn("updated---", (0, import_index_e390a0ae.g)(import_mapstore.selected), (0, import_index_e390a0ae.g)(import_mapstore.add_mode));
}
function draw_point(e) {
  const oalist = new Set(e.features.map((d) => d.properties.oa));
  const current = (0, import_index_e390a0ae.g)(import_mapstore.selected);
...
function geomean(c1, c2, thresh = 30) {
  c1 = (0, import_index_e390a0ae.g)(import_mapstore.mapobject).project(c1);
  c2 = (0, import_index_e390a0ae.g)(import_mapstore.mapobject).project(c2);
...
function draw_radius(center, points = 20) {
  ...
  var km = (0, import_index_e390a0ae.g)(import_mapstore.radiusInKm) / 2;
  var coordinates2 = [];
...
function circle_paint(clear = false) {
  ...
  if (import_mapstore.mapobject) {
    if (clear == true) {
      return (0, import_index_e390a0ae.g)(import_mapstore.mapobject).setPaintProperty("circle_layer", "circle-radius", 5);
    }
    const m2p = (meters, latitude) => meters / 0.075 / Math.cos(latitude * Math.PI / 180);
    (0, import_index_e390a0ae.g)(import_mapstore.mapobject).setPaintProperty("circle_layer", "circle-radius", {
      base: 2,
      stops: [
        [0, 0],
        [22, m2p((0, import_index_e390a0ae.g)(import_mapstore.radiusInKm) * 2e3, (0, import_index_e390a0ae.g)(import_mapstore.mapobject).getCenter().lat)]
      ]
    });
  }
}
function draw_polygon(e) {
  if (coordinates.length) {
    if (geomean(coordinates[0], [e.lng, e.lat])) {
      ...
      update(coordinates);
      console.log("--saving polygon", (0, import_index_e390a0ae.g)(import_mapstore.selected));
      coordinates = [];
      return 1;
    }
  }
...
async function simplify_query() {
  const last = (0, import_index_e390a0ae.g)(import_mapstore.selected)[(0, import_index_e390a0ae.g)(import_mapstore.selected).length - 1];
  ...
  (0, import_index_e390a0ae.g)(import_mapstore.mapobject).fitBounds(bbox, { padding: 200, animation: false, linear: true, duration: 200 });
  const oalist = [...last.oa];
  await new Promise((res) => setTimeout(res, 500));
  const features = (0, import_index_e390a0ae.g)(import_mapstore.mapobject).queryRenderedFeatures({
    layers: ["bounds"]
  }).filter((d) => oalist.includes(d.properties.oa));
  console.warn("features", features, (0, import_index_e390a0ae.g)(import_mapstore.mapobject).queryRenderedFeatures({
    layers: ["bounds"]
  }));
  if (!features.length) {
    return false;
