import { c as create_ssr_component, p as createEventDispatcher, h as escape } from "../../../../chunks/index-12fa369c.js";
...
<div id="${"select"}" class="${["svelte-10s1oab", ""].join(" ").trim()}">${selectedItem && !search ? `<a id="${"toggle"}" class="${"selected svelte-10s1oab"}"><span class="${"selection svelte-10s1oab"}">${escape(selectedItem[label])} ${group ? `<small class="${"svelte-10s1oab"}">${escape(selectedItem[group])}</small>` : ``}</span>
		<span class="${"button close svelte-10s1oab"}">\xA0</span></a>` : `<a id="${"toggle"}" class="${"svelte-10s1oab"}"><span class="${"svelte-10s1oab"}">${escape(placeholder ? placeholder : "Select one")}</span>
		<span class="${[
