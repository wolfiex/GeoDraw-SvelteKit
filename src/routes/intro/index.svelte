<script>
	...
	import { init_db, load_db, query_db } from '../util_db.js';
	...
	let areas = ['Output_Areas', 'Middle_Layer_Super_Output_Areas', 'Lower_Layer_Super_Output_Areas'];
	...
	$mapfunctions = [
		{
			...
			callback: function (e) {
				...
				var d = $mapobject.queryRenderedFeatures(e.point, { layers: ['select_layer'] })[0]
					.properties;
				...
				if (areaselect.length > 4) {
					// alert_notify('stop')
					alert_notify = 'You have selected too many areas. Please deselect some.';
				} else {
					alert_notify = false;
				}
			}
		},
		{
			...
			callback: (e) => {
				if (e.features.length > 0) {
					...
					$mapobject.setFeatureState({ source: 'selector', id: hoveredStateId }, { hover: true });
				}
			}
		},
		{
			...
			callback: () => {
				if (hoveredStateId !== null) {
					$mapobject.setFeatureState({ source: 'selector', id: hoveredStateId }, { hover: false });
				}
				hoveredStateId = null;
			}
		}
...
{#if alert_notify}
	<div style="position:relative;z-index:0;display:inline-block;margin-left:20%;width:60%;">
		<InlineNotification
			caption=""
...
<main>
	...
	<div class="bx--grid">
		<div class="bx--row">
			<div class="bx--col-md-4" style="float:left">
				...
				>
					<p slot="s1">
						Drag or Zoom the map to navigate. Locate areas of interest and <b
							>select them by clicking on the map</b
						>. Once done, press the button below.
						<br /><br />
						<Button size="field" kind="secondary" on:click={debounce('s2')} disabled={alert_notify}
							>Next</Button
						>
					</p>
      ...
      items={[
		  ...
		  {id:'poly', text:'Draw Polygon',active:true},
    ...
    />
					...
					>
						...
						<FormGroup legendText="">
							<Tooltip direction="bottom" tabIndex={0} triggerText="SELECT THE TYPE OF AREA">
								<p style="color:white">
									There are a number of different area types that can be selected. These are
									generally split based on poulation... --insert table here--
								</p>
	...
	Create
							...
							>
								{#each areas as ar}
									<RadioButton id={ar} labelText={ar.replaceAll('_', ' ')} value={ar} />
								{/each}
							</RadioButtonGroup>
						</FormGroup>
					</div>
